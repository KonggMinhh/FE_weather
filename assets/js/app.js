...
searchField.addEventListener("input", function () {
    ...
    if (searchField.value) {
        searchTimeout = setTimeout(() => {
            fetchDataSearch(url.geo(searchField.value), function (locations) {
...
export const updateWeather = function (lat, lon) {
    ...
    // * * * Current Weather * * *
    fetchDataCurrent(url.currentWeather(lat, lon), function (currentWeather) {
        const {
            weather,
            dt: dateUnix,
        ...
        // * * * Today Weather * * *
        fetchDataPollution(url.airPollution(lat, lon), function (airPollution) {
            const [
                {
                    main: { aqi },
                    components: { no2, o3, so2, pm2_5 },
                },
            ] = airPollution.data.list;
            console.log(airPollution, aqi);
                    ...
                    <div class="card card-sm highlight-card one">
                        ...
                        <div class="card card-sm highlight-card">
                            ...
                            <div class="wrapper">
                                ...
                                <p class="title-1">
                                    ${
                                        Math.round(feels_like / 10) > 0
                                            ? `${Math.round(feels_like / 10)}`
                                            : Math.round(feels_like / 10)
                                    }&deg;<sub>c</sub>
                                </p>
                            </div>
        ...
        // * * * Hourly Forecast * * *
        fetchDataForecast(url.forecast(lat, lon), function (forecast) {
            const {
                list: forecastList,
                city: { timezone },
            } = forecast;
            ...
            for (const [index, data] of forecastList.entries()) {
                ...
                <div class="card card-sm slider-card">
                    ...
                    <img
                        ...
                        title="${description}"
                    />
                    <p class="body-3">${Math.round(temp / 10)}&deg;</p>
                </div>
                `;
                ...
                windLi.innerHTML = `
                    <div class="card card-sm slider-card">
                        ...
                        />
                        <p class="body-3">${
                            Math.round(module.mps_to_kmh(windSpeed) * 10) / 10
                        } km/h</p>
                    </div>
                `;
                ...
                li.innerHTML = `
                    <div class="icon-wrapper">
                        ...
                        <span class="span">
                            <p class="title-2">
                            ${
                                Math.round(temp_max / 10) > 0
                                    ? `${Math.round(temp_max / 10)}`
                                    : Math.round(temp_max / 10)
                            }&deg;</p>
                        </span>
                    </div>
